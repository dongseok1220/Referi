...
def construct_prompt(args, dataset, fewshot): 
    ...
    if args.task == "math500":
        ...
        for subject in tqdm(subjects): 
            ...
            for idx, entry in enumerate(test_data):
                ...
                user_prompt += start_prompt + format_example_fixed(entry['question'], entry['options']) + '\n\n'
                model_outputs = res[idx]['resps'][0]
                for i, output in enumerate(model_outputs):
                    user_prompt += f"Response {i}: {output}\n"
    ...
    elif args.task == "musr_location" or args.task == "musr_efficiently": 
...
if __name__ == "__main__":
    ...
    parser.add_argument("--num_examples", type=int, default=-1, help="num")
