...
def construct_prompt(args): 
    ...
    prommpt_path = f"result/{args.task}/{args.model}/{args.task}_{args.shot_type}.jsonl"
    prompts = load_model_outputs(prommpt_path)
   
    output_res_path = f"{args.input_dir}/{args.task}/{args.model}/{args.task}_few.jsonl"
    ...
    for r, prompt in zip(res, prompts): 
        ...
        sample = {"idx": idx,"prompt": message,"entry": entry,"model_outputs": model_outputs}
...
if __name__ == "__main__":
    ...
    parser.add_argument("--num_examples", type=str, default=-1, help="output_dir")
